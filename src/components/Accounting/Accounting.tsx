...
import { useAppStore, MonthlyAnalysis } from '../../stores/appStore';
...
export const Accounting: React.FC = () => {
  ...
  const { loadTransactions, loadCategories, monthlyAnalysis, loadMonthlyAnalysis } = useAppStore();
  ...
  }, [view, selectedYear, selectedMonth, loadMonthlyAnalysis]);
  ...
  const chartData = useMemo(() => {
    ...
    return { expensePieData, incomePieData, comparePercent, topCategories };
  }, [monthlyAnalysis]);

  const totalIncome = (monthlyAnalysis as MonthlyAnalysis)?.total_income || 0;
  const totalExpense = (monthlyAnalysis as MonthlyAnalysis)?.total_expense || 0;

  if (view === 'month' && !monthlyAnalysis) {
